...
double* GetMatrixFromFile(char *path, int& rows, int& columns)
{
	std::ifstream file;
	...
	double* fileMatrix = new double[0];
	std::string fileLine;
	rows = 0;
	columns = 0;
...
double* GetMatrixFromConsoleInput(int& rows, int& columns)
{
...
double* GetRandomMatrix(int& rows, int& columns, double minInclusive, double maxExclusive)
{
	printf("RU/EN - Введите кол-во строк // Set rows amounts: ");
	scanf("%d", &rows);
...
double* GetIdentityMatrix(int dims)
{
